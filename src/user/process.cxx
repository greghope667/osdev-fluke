...
Process::load_flat_binary(const char* binary, usize size)
{
    ...
    if (vm.alloc_fixed(prog, program_size, PROT_EXEC|PROT_READ|PROT_WRITE).is_err())
...
Process::load_init_elf(const char* elf)
{
    ...
    for (int i=0; i<ph_count; i++) {
        ...
        if (p.p_type == PT_LOAD) {
            ...
            if (vm.alloc_fixed(p.p_vaddr, memsz, PROT_READ|PROT_WRITE|PROT_EXEC).is_err())
                panic("vm_alloc_fixed failed");
    ...
    if (vm.alloc_fixed(stack - stack_size, stack_size, PROT_READ|PROT_WRITE).is_err())
        panic("vm_alloc_fixed failed");
